...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure udPortClick(Sender: TObject; Button: TUDBtnType);
  private
    { Private declarations }
    FRpcCaller: TRpcCaller;
    FRpcFunction: IRpcFunction;
    procedure AddMessage(const Msg: string);
    function Login(user, password: String): String;
  public
    { Public declarations }
  end;
...
procedure TForm1.udPortClick(Sender: TObject; Button: TUDBtnType);
begin
...
function TForm1.Login(user,password:String):String;
var
...
begin
  ...
  FRpcFunction.Clear;
  FRpcFunction.ObjectMethod := 'session.login_with_password';
  FRpcFunction.AddItem(user);
  FRpcFunction.AddItem(password);
  ...
  try
    ...
    if RpcResult.IsError then
      ...
      AddMessage(RpcResult.AsStruct['Value'].AsString);
  except
    on E: Exception do
